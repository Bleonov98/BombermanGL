...
void Blob::MoveAnimation(float dt)
{
	moveAnimationTime += dt;
	if (moveAnimationTime >= moveAnimationInterval) {
		SetTexture(ResourceManager::GetTexture("blob_" + std::to_string(animFrame)));
...
void Blob::DeathAnimation(float dt)
{
	deathDelay -= dt;
	if (deathDelay <= 0.0f) {
		DeleteObject();
	...
	if (deathAnimationTime >= deathAnimationInterval) {
		SetTexture(ResourceManager::GetTexture("blob_death_" + std::to_string(animFrame)));
